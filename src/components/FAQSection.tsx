            ...
            {faqs.map((faq, index) => (
                <Card key={index} className="bg-white border-light-blue-200 hover:shadow-md transition-all duration-300">
                  <CardContent className="p-0">
                    <Button
                        variant="ghost"
                        className="w-full p-6 text-left hover:bg-light-blue-50"
                        onClick={() => toggleFAQ(index)}
                    >
                      <div className="flex items-center justify-between w-full gap-4">
                        <div className="text-base sm:text-lg font-semibold text-gray-800 text-left break-words flex-1 overflow-hidden whitespace-normal">
                          {faq.question}
                        </div>
                        <div
                            className="text-light-blue-600 text-xl transition-transform duration-200 shrink-0"
